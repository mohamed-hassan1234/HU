  ...
  return (
    <>
      ...
      <div className="panel overflow-hidden">
        ...
        {loading ? (
          <div className="p-8 text-center text-sm text-stone-500">Loading records...</div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-stone-200 text-sm">
              <thead className="bg-stone-50">
                <tr>
                  {columns.map((column) => (
                    <th key={column.key} className="whitespace-nowrap px-4 py-3 text-left font-semibold text-stone-600">{column.label}</th>
                  ))}
              ...
              <tbody className="divide-y divide-stone-100 bg-white">
                {visibleRows.map((row) => (
                  <tr key={row._id}>
                    {columns.map((column) => (
                      <td key={column.key} className="whitespace-nowrap px-4 py-3 text-stone-700">{Array.isArray(row[column.key]) ? row[column.key].join(', ') : row[column.key]}</td>
                    ))}
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
          <form onSubmit={submit} className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-md bg-white p-5 shadow-soft">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-stone-900">{editing ? `Edit ${title}` : `Add ${title}`}</h2>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {fields.map((field) => (
                <label key={field.name} className={field.type === 'textarea' ? 'sm:col-span-2' : ''}>
                  <span className="mb-1 block text-xs font-semibold uppercase text-stone-500">{field.label}</span>
                  {field.type === 'select' ? (
